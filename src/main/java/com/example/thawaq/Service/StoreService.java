...
//            throw new ApiException("Store not found");
    ...
    public List<Store> getRestaurantsYouLike(User user){
        Client client = clientRepository.findClientById(user.getId());
        if (client == null){
            throw new ApiException("User must be a Client");
        }
        List<Store> stores = new ArrayList<>();
    ...
    public List<Store> getCafesYouLike(User user){
        Client client = clientRepository.findClientById(user.getId());
        if (client == null){
            throw new ApiException("User must be a Client");
        }
    ...
    public List<Store> getCafesYouMayLike(User user){
        Client client = clientRepository.findClientById(user.getId());
        if (client == null){
            throw new ApiException("User must be a Client");
        }
    ...
    public List<Store> getRestaurantYouMayLike(User user){
        Client client = clientRepository.findClientById(user.getId());
        if (client == null){
            throw new ApiException("User must be a Client");
        }
        ...
        List<Category> categories = new ArrayList<>();
        for (Favorite favorite : favorites) {
            if (favorite.getMenu().getBranch().getStore().getTypeOfActivity().equals("RESTAURANT")){
                categories.add(favorite.getMenu().getCategory());
            }
        }
