...
public class UserService {
    ...
    public void clientUpdate(User log ,ClientDTO clientDTO) {
        if (!log.getId().equals(clientDTO.getId())) {
    ...
    public void storeAdminUpdate(User log,StoreAdminDTO storeAdminDTO) {
        if (!log.getId().equals(storeAdminDTO.getId())) {
            throw new ApiException("Client id mismatch");
        }
    ...
    public void expertUpdate(User log ,ExpertDTO expertDTO){
        if (!log.getId().equals(expertDTO.getId())) {
            throw new ApiException("Client id mismatch");
        }
    ...
    //// ahmed almroba
    public void blockStore(Integer storeId) {
...
///////////ahmed almroba
    public void unBlockStore(Integer storeId) {
        Store store = storeRepository.findStoreById(storeId);
        if (store == null) {
            throw new ApiException("Store not found");
        }
    ...
    //Jana
    public void activateStoreAdmin(Integer userId,Integer storeAdminId) {
        StoreAdmin storeAdmin = storeAdminRepository.findStoreAdminById(storeAdminId);
        User u = userRepository.findUserById(userId);
        if (storeAdmin == null) {
            throw new ApiException("Store Admin not found");}
        if(!u.getRole().equals("ADMIN")){
            throw new ApiException("You are not allowed to unblock this store admin");}
        storeAdmin.setActive(true);
        storeAdminRepository.save(storeAdmin);}

    //Jana
    public void DeactivateStoreAdmin(Integer userId,Integer storeAdminId) {
        StoreAdmin storeAdmin = storeAdminRepository.findStoreAdminById(storeAdminId);
        User u = userRepository.findUserById(userId);
        if (storeAdmin == null) {
            throw new ApiException("Store Admin not found");}
        if(!u.getRole().equals("ADMIN")){
            throw new ApiException("You are not allowed to block this store admin");}
        storeAdmin.setActive(false);
        storeAdminRepository.save(storeAdmin);}
}
